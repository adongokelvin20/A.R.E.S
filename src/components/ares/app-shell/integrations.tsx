...
function WhatsAppConnectModal({ onClose, onConnected }: { onClose: () => void; onConnected: () => void }) {
  ...
  // Load the Meta config from the backend
  useEffect(() => {
          ...
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
            <div className="flex items-start gap-2">
              ...
              <div className="text-xs leading-relaxed text-amber-800">
                ...
                <p className="mt-1">
                  Meta WhatsApp Embedded Signup needs a one-time server configuration. The platform owner must set
                ...
                <p className="mt-2">Once configured, customers connect WhatsApp with a single click.</p>
              </div>
            </div>
          </div>
        </div>
  ...
  if (showQr && qr) {
    const expired = qrExpiresIn <= 0;
    return (
      <ModalShell onClose={onClose} title="Scan to connect WhatsApp">
        <div className="space-y-4">
          <div className="flex flex-col items-center">
            <div className={`rounded-2xl border bg-white p-4 ${expired ? "opacity-40" : ""}`}>
              <img src={qr.qrCode} alt="WhatsApp connect QR code" className="h-56 w-56" />
            </div>
            {expired ? (
              <>
                <div className="mt-4 text-xs font-medium text-rose-600">QR code expired</div>
                <button
                  onClick={generateQr}
                  disabled={qrLoading}
                  className="mt-2 inline-flex items-center gap-1.5 rounded-lg bg-ares-navy px-4 py-2 text-xs font-semibold text-white hover:bg-ares-sea-deep disabled:opacity-60"
                >
              ...
              <>
                <div className="mt-4 flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  Waiting for scan… ({qrExpiresIn}s)
                </div>
                <p className="mt-2 max-w-xs text-center text-[11px] leading-relaxed text-muted-foreground">
                  Open your phone camera, scan the QR code, and complete Meta's signup. This page updates automatically when WhatsApp is connected.
                </p>
              </>
            )}
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col gap-2">
          ...
          >
            ...
            {qrLoading ? "Generating…" : "Scan QR code"}
          </button>
        </div>
        ...
        <div className="flex items-center gap-1.5 rounded-lg bg-ares-mist px-3 py-2 text-[11px] text-muted-foreground">
          ...
          Official Meta Cloud API · fully policy-compliant
        </div>
