...
> = {
  WHATSAPP_META: {
    ...
    requiredFields: [],
    embeddedSignup: true,
    // WhatsApp connects via Embedded Signup callback -- never via manual credential POST.
  },
  PAYMENT_PAYSTACK: {
    name: "Paystack",
  ...
  try {
    ...
    if (def.embeddedSignup) {
      return NextResponse.json(
        { error: "This integration connects via Embedded Signup. Use the Connect WhatsApp button." },
        { status: 400 }
      );
    }
